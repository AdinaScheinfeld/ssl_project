# Script to use Selma data to finetune model created using Wu data
...
from wu_transforms import get_finetune_train_transforms, get_finetune_val_transforms
...
if __name__ == '__main__':
    ...
    train_ds = torch.utils.data.ConcatDataset([
        SegPatchDataset(Path(config['patch_dir']) / 'train' / subfolder, get_finetune_train_transforms())
        for subfolder in os.listdir(Path(config['patch_dir']) / 'train')
    ])
    val_ds = torch.utils.data.ConcatDataset([
        SegPatchDataset(Path(config['patch_dir']) / 'val' / subfolder, get_finetune_val_transforms())
        for subfolder in os.listdir(Path(config['patch_dir']) / 'val')
    ])
