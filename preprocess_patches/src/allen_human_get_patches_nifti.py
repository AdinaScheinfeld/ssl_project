# allen_human_get_patches.py - Script to Extract Smaller Patches from 2D .tif Stacks and Create .tiff volumes
...
# function to create prefix for patch filenames
def create_allen_human_patch_prefix(nifti_path):
    ...
    stain = stain_match.group(1).lower() if stain_match else 'unknown'
...
# function to extract patches from nifti
def extract_patches_from_nifti(nifti_path, output_dir, prefix):

    print(f'Processing: {prefix} ({nifti_path})', flush=True)

    # use filename stem for deterministic seeding
    sample_id = os.path.splitext(os.path.basename(nifti_path))[0]
    ...
    # extract only first channel if multi-channel
    if data.ndim == 4:
        data = data[:, :, :, 0]
        print(f'[DEBUG] Loaded multi-channel nifti, using first channel only.', flush=True)

    # ensure volume is 3d
    if data.ndim != 3:
        raise ValueError(f'Expected 3D volume data, got shape {data.shape} for {nifti_path}.')
    
    # cast to uint16 and reorder axes to z, y, x
    data = data.astype(np.uint16)
    volume_full = np.transpose(data, (2, 1, 0))  # (x, y, z) -> (z, y, x)

    # compute padding so all dims are multiple of PATCH_SIZE
    height, width = volume_full.shape[1], volume_full.shape[2]
    pad_y = (PATCH_SIZE - height % PATCH_SIZE) % PATCH_SIZE
    pad_x = (PATCH_SIZE - width % PATCH_SIZE) % PATCH_SIZE
    pad_z = (PATCH_SIZE - volume_full.shape[0] % PATCH_SIZE) % PATCH_SIZE

    # pad
    volume_full = np.pad(volume_full, ((0, pad_z), (0, pad_y), (0, pad_x)), mode='constant', constant_values=0)
    padded_shape = (volume_full.shape[1], volume_full.shape[2])
    total_slices = volume_full.shape[0]
    ...
    if 2 * SKIP_Z_BORDER_BLOCKS >= num_blocks:
        print(f'[WARN] skip_z_border_blocks={SKIP_Z_BORDER_BLOCKS} removes all z blocks (only {num_blocks} available). Lower the value.', flush=True)
    ...
    for z0 in range(start_z, end_z, PATCH_SIZE):
        ...
        # extract volume block
        volume = volume_full[z0:z0+PATCH_SIZE, :, :]

        # thresholding
        print('Thresholding...', flush=True)
        threshold_a = block_otsu(volume)

        # skip outermost tiels in x/y if requested
        ...
        for y in range(y_start, y_stop, PATCH_SIZE):
            for x in range(x_start, x_stop, PATCH_SIZE):
                ...
                # local otsu threshold inside patch
    ...
    for saved_count, item in enumerate(selected[:to_save]):
        ...
        patch_path = os.path.join(output_dir, f'{prefix}_ps{PATCH_SIZE}_p_{saved_count}_cand{cand_idx}_z{z0}_y{y0}_x{x0}.nii.gz')

        # save as nifti
        nib.save(nib.Nifti1Image(patch_nifti, affine=np.eye(4)), patch_path)
        print(f'Saved random patch {saved_count} (cand idx {cand_idx} @ z{z0}, y{y0}, x{x0}) -> {patch_path}', flush=True)
    ...
    for root, dirs, files in os.walk(ROOT_DIR):
        for f in files:
            if f.lower().endswith(('.nii', '.nii.gz')) and not f.startswith('.'):
                ...
                prefix = create_allen_human_patch_prefix(nifti_path)
                samples.append((nifti_path, prefix))

    print(f'Discovered {len(samples)} nifti volumes under {ROOT_DIR}.', flush=True)
    return samples
...
def process_all(output_root):
    ...
    for nifti_path, prefix in get_all_sample_dirs():
        extract_patches_from_nifti(nifti_path, output_root, prefix)

# function to process just 1 sample
def process_single(nifti_path, prefix, output_dir):
    extract_patches_from_nifti(nifti_path, output_dir, prefix)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--single_dir', type=str, help='Path to directory with single nifti volume (.nii or .nii.gz)')
    ...
    parser.add_argument('--only_structure', type=str, choices=list(structures.keys()), help='Only process this structure (e.g. vessel)') # ignored, kept for compatibility
    ...
    if args.sample_index is not None:
        ...
        if not all_samples:
            print(f'No nifti volumes found under {ROOT_DIR}. Check directory layout and permissions.', flush=True)
            raise SystemExit(1)
