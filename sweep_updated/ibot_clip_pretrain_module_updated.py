# ibot_clip_pretrain_module_updated.py - Module for IBot pretraining
...
class IBOTCLIPPretrainModuleUpdated(pl.LightningModule):
    ...
    def __init__(self, config):
        ...
        # loss weights (targets) and warmup
        self.distill_weight = float(config['loss_weights']['distill_weight'])
        self.reconstruction_weight = float(config['loss_weights']['reconstruction_weight'])
        self.align_weight = float(config['loss_weights']['align_weight'])
        self.clip_weight = float(config['loss_weights']['clip_weight'])
        ...
        # warmup schedule for clip/align (epochs)
        loss_warmup_config = config.get('loss_warmup', {})
        self.clip_warmup_epochs = int(loss_warmup_config.get('clip_warmup_epochs', self.warmup_epochs))
        self.align_warmup_epochs = int(loss_warmup_config.get('align_warmup_epochs', self.warmup_epochs))

        # default effective weights to avoid attribute errors during warmup
        self._effective_clip_weight = float(self.clip_weight if self.clip_warmup_epochs <= 0 else 0.0)
        self._effective_align_weight = float(self.align_weight if self.align_warmup_epochs <= 0 else 0.0)
    ...
    def compute_clip_loss(self, image_embeds, text_embeds):
        ...
        logit_scale = self.logit_scale.exp().clamp(1e-3, 100.0) # constrain to reasonable range

        # ddp all-gather embeddings across gpus to increase negative sample pool
        if torch.distributed.is_available() and torch.distributed.is_initialized():
            world_size = torch.distributed.get_world_size()
            rank = torch.distributed.get_rank()

            # get local batch size
            local_batch_size = int(image_embeds.shape[0])

            # if rank has empty local batch, return zero loss
            if local_batch_size == 0:
                return image_embeds.new_tensor(0.0), logit_scale

            # gather local sizes as tensors then convert to ints
            local_batch_size_t = torch.tensor([local_batch_size], device=image_embeds.device, dtype=torch.int64)
            sizes_t = [torch.zeros_like(local_batch_size_t) for _ in range(world_size)]
            torch.distributed.all_gather(sizes_t, local_batch_size_t)
            sizes = [int(s.item()) for s in sizes_t]
            max_batch_size = int(max(sizes))
            pad_size = max_batch_size - image_embeds.size(0)
            if pad_size > 0:
                pad_image = torch.zeros((max_batch_size - image_embeds.size(1)), device=image_embeds.device, dtype=image_embeds.dtype)
                pad_text = torch.zeros((max_batch_size - text_embeds.size(1)), device=text_embeds.device, dtype=text_embeds.dtype)
                pad_image[:image_embeds.size(0)] = image_embeds
                pad_text[:text_embeds.size(0)] = text_embeds
            else:
                pad_image = image_embeds
                pad_text = text_embeds
            gather_image = [torch.zeros_like(pad_image) for _ in range(world_size)]
            gather_text = [torch.zeros_like(pad_text) for _ in range(world_size)]
            torch.distributed.all_gather(gather_image, pad_image)
            torch.distributed.all_gather(gather_text, pad_text)
            image_all = torch.cat([g[:s] for g, s in zip(gather_image, sizes)], dim=0)
            text_all = torch.cat([g[:s] for g, s in zip(gather_text, sizes)], dim=0)

            # build global labels (local samples align to same indices in concatenated all-gathered tensors)
            start = int(sum(sizes[:rank])) # starting index for local batch in all-gathered tensors
            idxs = torch.arange(start, start + local_batch_size, device=image_embeds.device)
            logits_image_to_text = (image_embeds @ text_all.T) * logit_scale
            logits_text_to_image = (text_embeds @ image_all.T) * logit_scale
            loss = (F.cross_entropy(logits_image_to_text, idxs) + F.cross_entropy(logits_text_to_image, idxs)) / 2.0
    ...
    def compute_ibot_loss(self, student_out_norm, teacher_out_norm, x, mask, student_feats_raw, student_image_embed, text_embed):
        ...
        # use warmed up align weight computed in on_train_epoch_start
        total_ibot_loss = (self.distill_weight * distill_loss + 
                           self.reconstruction_weight * recon_loss_l1 + 
                           self._effective_align_weight * alignment_loss)
    ...
    def on_train_epoch_start(self):
        ...
        # precompute current effective weights with warmup ramps
        def ramp_weight(current_epoch, warmup_epochs, target_weight):
            if warmup_epochs <= 0: 
                return target_weight
            return target_weight * min(1.0, float(current_epoch + 1) / float(warmup_epochs))
        ...
        self._effective_clip_weight = ramp_weight(self.current_epoch, self.clip_warmup_epochs, self.clip_weight)
        self._effective_align_weight = ramp_weight(self.current_epoch, self.align_warmup_epochs, self.align_weight)
    ...
    def shared_step(self, batch, batch_idx, is_train=True):
        ...
        # use warmed up weights for clip and align losses
        total_loss = ibot_loss + self._effective_clip_weight * clip_loss
        ...
        # log fixed, comparable validation metric (using canonical report weights)
        if not is_train:
            val_loss_report = (
    ...
    def configure_optimizers(self):
        ...
        # use gentler lr for text (when frozen) and normal lr for rest of model
        text_params = []
        other_params = []
        for name, param in self.named_parameters():
            if not param.requires_grad:
                continue
            (text_params if name.startswith('text_encoder') or name.startswith('text_proj') else other_params).append(param)
        param_groups = []
        if other_params:
            param_groups.append({'params': other_params, 'lr': self.lr})
        if text_params:
            param_groups.append({'params': text_params, 'lr': max(self.lr * 0.1, 1e-6)}) # smaller lr for text encoder
