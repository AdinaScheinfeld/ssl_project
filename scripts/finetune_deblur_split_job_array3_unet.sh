...
#SBATCH --output=/midtier/paetzollab/scratch/ads4015/temp_selma_deblur_preds_unet_random2_v2/logs/finetune_deblur_unet_%A_%a.out
#SBATCH --error=/midtier/paetzollab/scratch/ads4015/temp_selma_deblur_preds_unet_random2_v2/logs/finetune_deblur_unet_%A_%a.err
...
python /home/ads4015/ssl_project/src/finetune_deblur_split_unet.py \
  ...
  --weight_decay 1e-5 \
  --freeze_bn_stats 0 \
  --wandb_project selma3d_deblur_unet_rand \
  --num_workers 1 \
  --channel_substr ALL \
  --folds_json "$FJSON" \
  --fold_id "$FID" \
