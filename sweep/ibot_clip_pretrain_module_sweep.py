...
class IBOTCLIPPretrainModuleSweep(pl.LightningModule):
    ...
    def on_train_start(self):
        ...
        # show wandb parameters to plot
        if self.trainer.is_global_zero:
            for name in ['train_loss', 'train_clip_loss', 'train_ibot_loss', 'val_loss', 'val_clip_loss', 'val_ibot_loss']:
                self.logger.experiment.log({name: 0.0}, commit=False)
    ...
    def shared_step(self, batch, batch_idx, is_train=True):
        ...
        self.log(f'{log_prefix}_loss', total_loss, on_step=True, on_epoch=True, prog_bar=True, batch_size=x.shape[0], sync_dist=True, logger=True)
        self.log(f'{log_prefix}_clip_loss', clip_loss, on_step=True, on_epoch=True, batch_size=x.shape[0], sync_dist=True, logger=True)
        self.log(f'{log_prefix}_ibot_loss', ibot_loss, on_step=True, on_epoch=True, batch_size=x.shape[0], sync_dist=True, logger=True)
    ...
    def on_train_epoch_end(self):
        ...
        # # log train loss by epoch
        # if self.trainer.is_global_zero:
        #     train_loss = self.trainer.callback_metrics.get('train_loss')
        #     try:
        #         train_loss_scaler = float(train_loss.detach().cpu().item()) if train_loss is not None else None
        #     except Exception:
        #         train_loss_scaler = None

        #     self.logger.experiment.log({
        #         'epoch': int(self.current_epoch),
        #         'train_loss': train_loss_scaler
        #     })
    ...
    def on_validation_epoch_end(self):
        ...
        # # log val loss by epoch
        # if self.trainer.is_global_zero:
        #     val_loss = self.trainer.callback_metrics.get('val_loss')
        #     try:
        #         val_loss_scaler = float(val_loss.detach().cpu().item()) if val_loss is not None else None
        #     except Exception:
        #         val_loss_scaler = None

        #     self.logger.experiment.log({
        #         'epoch': int(self.current_epoch),
        #         'val_loss': val_loss_scaler
        #     })
        ...
        # push key metrics to wandb summary for sweeps
        for key in ['train_loss', 'train_clip_loss', 'train_ibot_loss', 'val_loss', 'val_clip_loss', 'val_ibot_loss']:
            value = c.get(key) or c.get(f'{key}_epoch')
            if value is not None:
                self.logger.experiment.summary[key] = float(value.item())
