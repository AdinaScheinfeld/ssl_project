# ibot_clip_pretrain_module_sweep.py - Module for IBot pretraining
...
class IBOTCLIPPretrainModuleSweep(pl.LightningModule):
    ...
    def __init__(self, config):
        ...
        self.base_mask_ratio = config['model']['mask_ratio'] # original mask ratio
        self.mask_ratio = self.base_mask_ratio # percentage of voxels to mask
        ...
        self.best_metrics = {
            ...
            'train_loss_norm': float('inf'),
            'val_loss_norm': float('inf')
        }
    ...
    def compute_clip_loss(self, image_embeds, text_embeds):
        ...
        # normalise image and text embeddings
    ...
    def compute_ibot_loss(self, student_features, teacher_features, x, mask, student_image_embed, text_embed):
        ...
        # return losses
        return total_ibot_loss, distill_loss, recon_loss, alignment_loss, student_recon
    ...
    def shared_step(self, batch, batch_idx, is_train=True):
        ...
        ibot_loss, distill_loss, recon_loss, alignment_loss, student_recon = self.compute_ibot_loss(
            student_out, teacher_out, x, mask, student_image_embed, text_embed)
        ...
        self.log(f'{log_prefix}_clip_loss', clip_loss, on_step=is_train, on_epoch=True, batch_size=x.shape[0], sync_dist=True)
        self.log(f'{log_prefix}_ibot_loss', ibot_loss, on_step=is_train, on_epoch=True, batch_size=x.shape[0], sync_dist=True)

        # compute normalized losses for logging
        world = self.trainer.world_size if hasattr(self.trainer, 'world_size') else 1
        accum = self.trainer.accumulate_grad_batches if hasattr(self.trainer, 'accumulate_grad_batches') else 1
        local_batch_size = x.shape[0]
        effective_global_batch_size = int(local_batch_size * max(1, world) * max(1, accum))

        clip_loss_adjusted = clip_loss - torch.log(torch.tensor(float(effective_global_batch_size), device=clip_loss.device))
        total_loss_normalized = distill_loss + recon_loss + alignment_loss + self.clip_weight * clip_loss_adjusted

        self.log(f'{log_prefix}_clip_loss_norm', clip_loss_adjusted, on_step=is_train, on_epoch=True, batch_size=x.shape[0], sync_dist=True)
        self.log(f'{log_prefix}_loss_norm', total_loss_normalized, on_step=is_train, on_epoch=True, batch_size=x.shape[0], sync_dist=True)
    ...
    def on_validation_epoch_end(self):
        ...
        self.update_best_metrics('train_loss_norm', c.get('train_loss_norm'))
        ...
        self.update_best_metrics('val_loss_norm', c.get('val_loss_norm'))
    ...
    def on_fit_end(self):
        ...
        if hasattr(self, 'logger') and hasattr(self.logger, 'experiment'):
            ...
            table = wandb.Table(columns=[
                ...
                'best_train_loss_norm', 'best_val_loss_norm',
            ...
            table.add_data(
                ...
                self.best_metrics['train_loss_norm'],
                self.best_metrics['val_loss_norm'],
