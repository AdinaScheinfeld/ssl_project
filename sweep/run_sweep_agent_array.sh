#!/bin/bash
#SBATCH --job-name=lsm_sweep_array
#SBATCH --output=logs/wandb_array_%A_%a.out
#SBATCH --error=logs/wandb_array_%A_%a.err
...
SWEEP_ID="$1"
if [ -z "$SWEEP_ID" ]; then
  echo "ERROR: Provide SWEEP_ID (e.g., adinas-wcm/ibot-clip-pretrain-lsm-all/abc123xy)"; exit 1
fi
...
export PRETRAIN_SWEEP_DIR="/ministorage/adina/pretrain_sweep"
...
export WANDB_DIR="/ministorage/adina/pretrain_sweep/wandb"
...
export BASE_CONFIG="/home/ads4015/ssl_project/configs/all_datasets_clip_pretrain_2_config.yaml"
